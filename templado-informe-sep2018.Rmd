...
knitr::opts_chunk$set(warning=FALSE, echo = FALSE, message = FALSE, dpi=300, 
                      fig.path='~/metagenomics/phyloseq_in_X04/figures/'
...
# mystations <- strsplit(rownames(df), "_")
# mystations <- sapply(mystations, "[", c(3))

init.contigs <- read.csv("/Users/cigom/metagenomics/phyloseq_in/initial.contigs", sep = "\t" )

plotdat <- cbind(mystations, init.contigs, mydf)

names(plotdat) <- c("Estacion", "Total", "Buena Calidad", "Alineamiento", "Barcodes")
rownames(plotdat) <- NULL

# melt files as follow

plotdat_m <- reshape2::melt(plotdat, id.vars = "Estacion", variable.name = "Etapa", value.name = "Secuencias")

plotdat_m$Etapa <- factor(plotdat_m$Etapa, levels = c("Total", "Buena Calidad", "Alineamiento", "Barcodes"), ordered = T)

seqs_barplot <- ggplot(plotdat_m, aes(x=Estacion, y=Secuencias, fill=Etapa)) + 
...
# ggsave(seqs_barplot, filename = "figura3.png", dpi = 100) # or save it
...
biom.dir <- c("/Users/cigom/metagenomics/phyloseq_in_X04")

setwd(biom.dir)
# An example of group file could be created as using the files file inputed in mothur
system("cat cigom.files | awk '{print $1}' > groups")
system("cat cigom.files | awk '{print $1}' | cut -d'_' -f1,2,3,4,5 | sed 's/_/ /g' > factors")
system("paste groups factors | tr '\t' ' ' > cigom.groups")
# | sed '1iID Run Cruice Estación Barcode SampleType' |
...
write.csv(save[,-c(4,18)], file = "taxones_identificados")
```
