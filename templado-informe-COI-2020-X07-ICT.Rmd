...
physeq <- subset_samples(phyloseq, Crucero == set) 
...
```{r}
ict_sam <- sample_data(physeq)
ict_sam$Tipo <-  sub('.*(?=.$)', '', ict_sam$Estación, perl=T)
ict_sam[ict_sam$Tipo == 'H','Tipo'] <- 'Huevos'
ict_sam[ict_sam$Tipo == 'L','Tipo'] <- 'Larvas'

sample_data(physeq) <- ict_sam
...
* Figura 5. Composición de la comunidad a nivel Filo (Barplot).
* Figura 6. Composición de la comunidad a nivel Clase (Barplot).
* Figura 7. Composición de la comunidad a nivel Orden (Heatmap)
* Figura 8.  Composición de las comunidades a nivel Familia a lo largo de las estaciones. espectivamente, se indican el nivel de Filos y Clase en las columnas del panel derecho (Heatmap)
* Figura 9. Especies más abundantes a lo largo de todas las estaciones. Se muestran aquellos taxones que representan al menos 10% de la abundancia total por muestra.
...
* Figura 12. Analisis de diversidad por estación. Se muestran los índices de diversidad calculados para la comunidad de zooplancton cada estación.
...
names <-  c("Muestras","Inicial", "Filtrado", "denoisedF", 
                     "denoisedR", "Empalme","tabled","Final")

names(track_out) <- names



track_out %>%
  filter(grepl(set, Muestras)) %>%
  mutate(Muestras = sapply(strsplit(Muestras, "-"), '[', 2)) %>%
  arrange(desc(Inicial)) %>%
  mutate(Muestras = factor(Muestras, Muestras)) %>%
  select(-tabled) %>%
  pivot_longer(-Muestras, names_to = 'Etapa', values_to = 'Secuencias') %>%
  mutate(Etapa = factor(Etapa, levels = names, ordered = T)) %>%
  ggplot(aes(x = Muestras, y = Secuencias)) + 
    geom_col(position = position_identity(), width = 0.7, 
             aes(fill = Etapa), alpha = 0.7, color = 'black') +
    scale_fill_brewer(palette = "Greens", direction = 1) +
    labs(x="Muestras", y = "Secuencias") + theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust=1, size = 7), axis.text.y = element_text(size = 7)) +
    coord_flip() +
  geom_hline(aes(yintercept = min(Secuencias)), linetype = "dashed", alpha = 0.9) -> track_plot

print(track_plot)

ggsave(track_plot, filename = paste0(path,"/", "track_plot.png") )

```

```{r Figura 4 empalme, echo = FALSE, fig.align = "center", fig.cap = "Figura 4. Tamano del empalme y amplicón generado", out.width='\\textwidth', fig.height=8, fig.width=8 }
...
pgrid <- grid.arrange(p2, empty, p1, p3,
                 ncol=2, 
                 nrow=2, 
                 widths=c(3, 1), 
                 heights=c(1, 3))

ggsave(pgrid, filename = paste0(path,"/", "empalme.png") )

```

```{r remove non-zooplankton and sporious features}
...
physeq %>% 
  subset_taxa(Reino == "Animals") %>%
  subset_taxa(!Clase %in%  cleanClass) %>%
  prune_taxa(taxa_sums(.) > 0,.) -> physeq



write.table(otu_table(physeq), 
            paste0(path, 'run018_test01_ASVs_animalia_count.table'))

write.table(tax_table(physeq), 
            paste0(path, 'run018_test01_ASVs_animalia.taxonomy'))

# Rscript --vanilla estimate_richness.R *_animalia_count.table

```

```{r write.table, eval=T}
save <- psmelt(physeq)
colnames(save)[1] <- "ASV"
write.csv(save, 
          file = paste0(path, "/", 
                        set, "_taxones_identificados.csv"), 
...
Debido a que el número de secuencias para cada muestra individual fue diferente. Con el objetivo de comparar las diferencias entre las muestras, se estandarizó la matriz de distribución de taxones (eg. OTUs, ASVs) en abundancia relativa (RA) basada en la siguiente ecuación: $$RA_{ij} = \frac{S_{ij}}{\sum_{j=1}^{n} S_{ij}}*100$$
...
La tabla adjunta (`r paste0(set, "_taxones_identificados.csv")`) contiene información desglosada de los taxones presentes en este crucero.

Para obtener una perspectiva biológica de la las comunidades se calculó la abundancia de los amplicones colapsados a diferentes niveles taxonómicos. El siguiente grupo de figuras muestra en mapas y graficos de barras de calor la abundancia aglomerada de los taxones por estación a lo largo de los niveles taxonómicos Filo, Clase, Orden y Familia.

```{r Figura 5. bar.phy, fig.align = "center", fig.cap = "Figura 5. Composición de las comunidades a nivel Filo a lo largo de las estaciones. La categoria 'Others' contiene ASVs sin asignacion taxonomica", out.width='\\textwidth', fig.width=8}
...
agglom_rank <- function(physeq, taxrank, filter = 0.01, NArm = FALSE) {
  ...
  # work on the dplyr programming style
  # mdf <- psmelt(Rank) %>%
  #   mutate(Others = ifelse(is.na(taxrank), 'Others', taxrank)) %>%
  #   mutate(Others = ifelse(Abundance > filter, 'Others', taxrank))
...
caption <- paste0("La categoria 'Others' contiene ASVs sin asignación taxonómica")

mdf <- agglom_rank(physeq, taxrank, fra)
#mdf[, wrap := ifelse(Abundance > 1, 'A','B')]
#mdf[Abundance < 1, Abundance:= -Abundance > 1]

mdf <- mdf %>% filter(Abundance > 0)
library(ggplot2)

colourCount <- length(unique(mdf$Group))
levs <- levels(factor(mdf$Group))
...
p = ggplot(mdf, aes(x = Estación, y = Abundance, fill = Group)) +
  scale_fill_manual(values = fvalues, name = taxrank)
...
```{r Figura 6. bar.Class, fig.align = "center", fig.cap = "Figura 6. Composición de las comunidades a nivel Clase a lo largo de las estaciones. La categoria 'Others' contiene ASVs sin asignacion taxonomica y con RA < 1", out.width='\\textwidth', fig.width=8}

taxrank <- 'Clase' # Rank a aglomerar
fra <- 1 # Filter RA

caption <- paste0("La categoria 'Others' contiene ASVs sin asignación taxonómica y con RA < ", fra, " %")
...
```{r Figura 7. Heatmap.Or, fig.align = "center", fig.cap = "Figura 7. Composición de las comunidades a nivel Orden a lo largo de las estaciones. La RA < 1 es removida", fig.height = 7, fig.width = 8}
...
Order <-tax_glom(physeq, taxrank = taxrank, NArm = T)

Order <- transform_sample_counts(Order, function(x) (x / sum (x) ) * 100)

dist <- distance(Order, method = "jaccard")
sampleOrder <- heatmap(as.matrix(dist))$colInd
sampleOrder <- colnames(otu_table(Order))[sampleOrder]
...
plot <- plot + guides(fill = guide_colorbar(barheight = unit(7, "cm"),
                                    ticks.colour = "black",
                                    frame.colour = "black"))
...
```{r Figura 8. Heatmap.Fam, fig.align = "center", fig.cap="Figura 8. Composición de las comunidades a nivel Familia a lo largo de las estaciones; respectivamente, se indican el nivel de Filos y Clase en las columnas del panel derecho", fig.width = 8, fig.height = 7}

taxrank <- "Familia"
fra <- 1 # Filter R

Family <-tax_glom(physeq, taxrank = taxrank, NArm = T)

Family <- transform_sample_counts(Family, function(x) (x / sum (x) ) * 100)

dist <- distance(Family, method = "jaccard")
sampleOrder <- heatmap(as.matrix(dist))$colInd
sampleOrder <- colnames(otu_table(Family))[sampleOrder]

Family <- filter_taxa(Family, function(x) sum(x) >= fra, TRUE)

taxaOrder = names(sort(taxa_sums(Family), decreasing = TRUE))


otu_table(Family)[otu_table(Family) == 0] <- NA
...
```{r Figura 9. Boxplot species, fig.align = "center", fig.cap = "Figura 9. Especies más abundantes a lo largo de todas las estaciones. Se muestran aquellos taxones que representan al menos 10% de la abundancia total por muestra.", fig.width = 8, fig.height = 7}
library(data.table)
library(ggplot2)
...
plot <- ggplot(dat[Abundance > 0 & Especie != 'NA'],
 ...
 labs(y = "Abundancia Relativa (%)", x ="Especies", title = paste0("Especies abundantes del crucero ", set),
      caption = paste0("Taxones que representan más del ", size, "% de la abundancia por muestra"))
...
plot <- NULL
plot <- rrplot +
  xlab("Número de secuencias") + ylab("Número de amplicones") + 
...
```{r Figura 12. Diversity, fig.align = "center", fig.cap = "Figura 12. Análisis is de diversidad por estación. Se muestran los índices de diversidad calculados para la comunidad de zooplancton cada estación.", out.width='\\textwidth', fig.width = 7, fig.height = 5}
...
obs_vs_chao <- estimate_richness(physeq)

rownames(obs_vs_chao) <-  sapply(strsplit(rownames(obs_vs_chao), "[.]"), `[`, 2)

op_plot <- ggscatter(obs_vs_chao, 
                     x = "Chao1", y = "Observed",
                     color = "Shannon",
                     label = rownames(obs_vs_chao), 
...
op_plot <- op_plot + gradient_color(pal_gsea("default")(10))

ggsave(op_plot, filename = "observed_vs_predicted_div.png", dpi = 300, path = path)

op_plot

```

La diversidad se obtuvo a través del cálculo de los índices de Shannon, Chao y observado de cada estación (Figura 12). La figura 12 muestra la dispersión de la riqueza de ASVs observados versus la riqueza predicha utilizando los estimadores Chao1 para las estaciones muestreadas. Los colores de los puntos representan el índice de diversidad Shannon para cada estación. La relación 1 indica entre la riqueza observada y predicha. Los valores de índices de diversidad alfa pueden encontrarse en la tabla 1.

```{r Tabla 1. Índices Diversidad, eval = T, echo = FALSE}
library(phyloseq)
...
# iDist <- phyloseq::distance(physeq.rr, method="jaccard")
# heatmap(as.matrix(iDist))

iMDS  <- ordinate(physeq, "PCoA", distance = "bray") # iDist

iMDS_screen <- plot_scree(iMDS)

p <- plot_ordination(rr, iMDS, justDF = F,
                     color="Transecto" )

p$layers <- p$layers[-1]
eigvec = iMDS$values$Eigenvalues
fracvar = eigvec[1:2]/sum(eigvec)
percvar = round(100 * fracvar, 1)
label_x <- paste0("Eje 1 [", percvar[1], "%]")
label_y <- paste0("Eje 2 [", percvar[2], "%]")


plot <- NULL
plot <- p + 
  geom_label(aes(label = Estación), fill = "white", alpha = 0.9) + theme(legend.position = "none") +
  scale_color_manual(values=getPalette(n)) +
  labs(x = label_x, y = label_y)

ggsave("Figura-13-PCoA.png", plot, path = path)
plot

```

```{r other methods, eval = FALSE, echo = FALSE}
...
plist = plyr::llply(as.list(ord_meths), function(i, physeq, dist){
        ...
        plot_ordination(physeq, ordi, "samples", color="Transecto")
}, physeq.rr, dist)
...
p = ggplot(pdataframe, aes(Eje_1, Eje_2, color=Transecto, fill=Transecto))
# p = p + geom_point(size=4)  + geom_polygon()
p = p + geom_label(aes(label = Estación), fill = "white", alpha = 0.9)
p = p + facet_wrap(~method, scales="free")
p = p + scale_fill_manual(values=getPalette(n))
p = p + scale_color_manual(values=getPalette(n))
p = p + theme(legend.position = "none") +
  labs(x = "Eje 1", y = "Eje 2")


# 
# p = plist$PCoA + scale_color_manual(values=getPalette(n))
# p = p + scale_fill_manual(values=getPalette(n))
# p = p + geom_label(aes(label = Estación), fill = "white", alpha = 0.9) + theme(legend.position = "none") + labs(x = axis_, y = "Eje 2")

# pca ----

samples <- samples[c(grep(set, samples$Crucero)),]

PCA <- prcomp(t(otu_table(physeq.rr)), scale. = FALSE)
percentVar <- round(100*PCA$sdev^2/sum(PCA$sdev^2),1)
sd_ratio <- sqrt(percentVar[2] / percentVar[1])

# barplot(PCA$x)

dtvis <- data.frame(PC1 = PCA$x[,1], PC2 = PCA$x[,2], samples)

pca_plot2 <- ggplot(dtvis, aes(PC1, PC2)) +
  geom_label(aes(label = Estación, fill = Transecto), alpha = 0.9) +
  labs(caption = '') +
...
2. Ratnasingham, S., & Hebert, P. D. (2007). BOLD: The Barcode of Life Data System (http://www. barcodinglife. org). Molecular ecology notes, 7(3), 355-364.
...
6. R Core Team (2018). R: A language and environment for
