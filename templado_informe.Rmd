...
if (identical(colnames(otu_table(biom)), rownames(mgroup))) { phyloseq <- merge_phyloseq(biom, mgroup) }
...
library(ape)
mtree <- list.files(biom.dir, full.names = TRUE, pattern = "cigom.0.03.rep.treein.tre")
t = read.tree(mtree[1])

# /// Merging a phyloseq object
if (identical(taxa_names(phyloseq), sort(t$tip.label))) { phyloseq = merge_phyloseq(phyloseq, t) 
    }  else { phyloseq = phyloseq }
...
```
..
```{r, echo = FALSE, fig.align = "center", fig.cap = "Curva de rarefacción que presenta la riqueza de OTUs a lo largo de las estaciones en relación al número de secuencias"}
...
rrplot <- ggrare(rr, step = 100, label = "Station", color="Station", se = FALSE, plot = FALSE) 
...
prichnes + geom_point(size=5, alpha=0.7) +
...
```{r, fig.align = "center", fig.cap = "Additional plots - otus (abundantes) en una/ dos estaciones unicamente"}

dp <- phyloseq::psmelt(rr)
colourCount = length(unique(tax_table(rr)[,3]))

library(ggpubr)

ggdotchart(dp, x = "Station", y ="Phylum",
 group = "Station", color = "Class",
...
```{r, echo = FALSE}
Hx.cls = subset_taxa(Animalia, Class=="Hexanauplia")
...
```{r, fig.align = "center", fig.cap="Hexanauplia tree (Family label in the xlab)"}
