...
tax.file = "run014_t2_ASVs.midori_unique_DB_0.wang.taxonomy"
...
ctb.file <- list.files(path, pattern = ctb.file, full.names = TRUE)
...
Ranks <- c("Reino", "Filo", "Clase", "Orden", "Familia", "Genero") # midori levels
...
glom <- transform_sample_counts(physeq, function(x) (x / sum (x) ) * 100 )
glom <- tax_glom(glom, taxrank = 'Genero')
# glom <- tax_glom(physeq2, taxrank = 'Genero')

# create dataframe from phyloseq object
dat <- data.table(psmelt(glom))
# convert Taxa to a character vector from a factor because R
dat$Genero <- as.character(dat$Genero)
# group dataframe by Taxa, calculate median rel. abundance
# dat[, median := median(Abundance, na.rm = TRUE), by = "Genero"]

# Change name to remainder of Genera less than 1%
dat[(Abundance <= 50), Genero := "Otros"]
# dat[(median <= 0.1), Genero := "Otros"]
...
p$layers <- p$layers[-1]
p <- p + geom_text(aes(label = Estación, size = 10)) + theme(legend.position = "none")
...
```{r Figura 14. Peces, fig.align = "center", fig.cap = "Figura 14. Taxones (nivel Familia) con varianza mayor a 1e-5 asignados a la Clase de _Actinoperigy_.", fig.width = 7, fig.height = 5}
library(phyloseq)
actii = subset_taxa(physeq2, Clase == "Actinopteri")
actii = prune_taxa(taxa_sums(actii) > 0, actii)
actii.Fgl = tax_glom(actii, taxrank="Familia")
...
G03.samples <- as.character(sample_data(actii.Fgl)$`Estación`[1:22]) # 23:45
