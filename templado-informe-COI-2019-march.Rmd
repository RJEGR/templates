...
```{r paths}
set <- 'X06' # ALSO 4 SAMPLES FROM X06
path <- "~/metagenomics/COI/run014"

qc.dir <- paste0(path, "/fastqc")
qc.files <- list.files(qc.dir, full.names = TRUE, pattern = set)

track.dir <- paste0(path, "/run014_20190305_COI_t2")
...
```{r inputs name}
ctb.file = "run014_t2_ASVs_count.table"
tax.file = "run014_t2_ASVs_midori_unique_DB_0.wang.wm.taxonomy"
...
taxa.obj <- read.csv(tax.file, header=FALSE, sep="\t", stringsAsFactors=FALSE)
