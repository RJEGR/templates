...
title: Análisis bioinformático de la diversidad de zooplancton en la zona epipelágica
  del Golfo de México usando el marcador x
...
dat <- read.table(ctb.file, header = T)
tax <- read_rdp(tax.file, header = T)
mtd <- makeMT(dat)
```
...
#identical(names(tax),rownames(mtd))
...
# if Subset_samples by cruice
# physeq <- subset_samples(phyloseq, Crucero == set) 
# Removing any taxa of abundance of zero
# physeq <- prune_taxa(taxa_sums(physeq) > 0, physeq) 

physeq <- subset_taxa(phyloseq, Reino == "Animalia")
physeq <- prune_taxa(taxa_sums(physeq) > 0, physeq)

# 1. Removing any abundance of zero
# 2. Remove ASVs not k greater than k (2L) 

keepTaxa = apply(X = as(otu_table(physeq), "matrix") > 0, # 1
                 MARGIN = 1, FUN = sum) > 2 # 2
...
Los datos provienen de muestras obtenidas en `r nsamp` estaciones que cubrió el crucero exploratorio `r set`. Para cada muestra se generaron amplicones xx. Los amplicones fueron secuenciados desde ambos extremos generando secuencias con una longitud de xxx pares de bases (pb) cada uno, en un secuenciador MiSeq de Illumina (del CICESE-CIGOM). Los siguientes resultados corresponden al análisis bioinformático de las secuencias generadas.

El análisis metagenómico se llevó a cabo en el cluster de cómputo OMICA-CICESE mediante el programa xxx versión xxx. La tubería del análisis fue adaptada de la sugerida por xxx et. al. (xxxx).
...
* Figura 14. Taxones con varianza mayor a 1e-5 asignados a la Clase de Actinoperigy. 
...
Phylum = subset_taxa(Phylum, Filo!="Undetermined")
...
bar <- NULL
bar <- plot_bar(Phylum, "Estación", fill = 'Filo')  +
       geom_bar(stat = "identity") +
  scale_fill_manual(values = getPalette(colourCount)) +
   ylab("Abundancia Relativa (%)") + coord_flip() +
  guides(fill=guide_legend(ncol=2)) +
   theme(panel.background = element_blank()) +
  labs(x = 'Estación', caption = "Abundancia relativa de taxones;  Se remueven las asignaciones 'Undetermined' por muestra")
...
png(paste0(path,"/", "Figura-7-Heatmap.Class.png"), units="px", width=2900, height=2600, res=400)
plot(plot, col=adjustcolor("black", alpha=0.2))
dev.off()
...
Order <- subset_taxa(Order, Orden!="NA")
...
plot <- plot + coord_flip() + theme(axis.text.x=element_text(angle = 45, hjust = 1, vjust = 1))
                                    
# plot + scale_y_discrete(position = "right")

png(paste0(path,"/", "Figura-8-Heatmap.Or.png"), units="px", width=2700, height=3200, res=400)
plot(plot, col=adjustcolor("black", alpha=0.2))
dev.off()

#
# extra figures
colourCount = length(unique(tax_table(Order)[,'Filo']))
#colourCount = nsamples(plotbar)
getPalette = colorRampPalette(brewer.pal(colourCount, "Paired"))
...
bar <- NULL
bar <- plot_bar(Order, x= "Orden", fill = 'Filo') +
...
png(paste0(path,"/", "Figura-8S-Heatmap.Or.png"), units="px", width=3600, height=2600, res=400)
plot(bar, col=adjustcolor("black", alpha=0.2))
dev.off()
...
```{r Figura 14. Peces, fig.align = "center", fig.cap = "Figura 14. Taxones (nivel Especie) con varianza mayor a 1e-5 asignados a la Clase de _Actinoperigy_.", fig.width = 7, fig.height = 5}
library(phyloseq)
...
plotbar = actii.glm
plotbar <- subset_taxa(actii.glm, Especie != 'NA')

colourCount = length(unique(tax_table(plotbar)[,'Especie']))
#colourCount = nsamples(plotbar)
...
bar <- NULL
bar <- plot_bar(plotbar, "Estación", fill = 'Especie')  +
       geom_bar(stat = "identity") +
  scale_fill_manual(values = getPalette(colourCount)) +
   ylab("Abundancia Relativa (%)") + coord_flip() +
  guides(fill=guide_legend(ncol=2)) +
...
```{r max.taxa, eval = FALSE, echo = FALSE}
...
subset_ = subset_taxa(physeq, Especie!="NA")
subset_sp <-tax_glom(subset_, taxrank="Especie")
subset_sp <- transform_sample_counts(subset_sp, function(x) (x / sum (x) ) * 100 )


max.taxa <- find.taxa(subset_sp, "Especie", "max", c("Estación", "Transecto"))

abundance <- data.frame(Abundance = apply(max.taxa[,-c(8:10)], 1, max))

identical(rownames(abundance), rownames(max.taxa))

max <- data.frame(max.taxa[,c('Especie', 'Estación', 'Transecto')], Abundancia=abundance[,1])
names(max)[2] <- 'Station'

```


```{r MAPA, eval = FALSE, echo = FALSE}
...
sample_size <- data.frame(id=names(sample_sums(physeq)), n_seqs = sample_sums(physeq))
max$id <- rownames(max) 

sample_size %>%
    as.tibble() %>%
    group_by(id) %>%
    inner_join(max, by = 'id') %>%
    group_by(Station) %>%
...
richness %>%
    ...
    inner_join(max, by = "Station") %>%
    select(Station, Transecto, Longitude, Latitude, Depth, 
                    Observed, Shannon, Especie, Abundancia) -> map_plot


# colour
library(RColorBrewer)
getPalette = colorRampPalette(brewer.pal(8, "RdYlBu"))
colourCount <- length((unique(map_plot$Especie)))
...
plot <- gg + 
  geom_point(data = map_plot, aes(Longitude, Latitude, color=Transecto, size = log10(n_seqs)), alpha = 1) +  
  geom_text_repel(data = map_plot,
                  aes(Longitude, Latitude, label=Station, color = Transecto),
...
# or ...
plot <- gg +
  geom_point(data = map_plot, aes(Longitude, Latitude, color = Transecto)) +
  geom_text_repel(data = map_plot,
                  aes(Longitude, Latitude, label=Station, color = Transecto),
                  segment.color = "grey50", size = 7
                  ) + scale_color_manual(values=getPalette(n)) +
...
plot <- gg + 
  geom_point(data = map_plot, aes(Longitude, Latitude, color=Shannon, size = Observed), alpha = 0.7) +  
...
ggplot(map_plot, aes(Shannon, Observed)) + 
  geom_point(aes(color=Abundancia), alpha=0.7, size=7, position='jitter') + # stat_smooth() +
  scale_color_continuous(low="blue", high="orange") +
...
plot <- NULL
plot <- gg + 
    geom_point(data = map_plot, aes(Longitude, Latitude, color=Especie, size = Observed), alpha = 0.7) +  
    scale_color_manual(values = c(getPalette(colourCount))) +  
    ggtitle("") +
    geom_text_repel(data = map_plot,
                  aes(Longitude, Latitude, label=Station),
        ...
        panel.background = element_blank(), legend.position = "right") +
        labs(#size = "Abundancia\nRelativa (%)",
             color = "Especie", subtitle = paste0("Especies mas abundantes del Crucero ", set),
             caption = 'Se cura el nivel especie: removiendo asignaciones NA y aglomerando las asignaciones al nombre común.')

png(paste0(path,"/", "Figura-map2.png"), units="px", width=5000, height=3500, res=400)
plot(plot, col=adjustcolor("black", alpha=0.2))
dev.off()
```
