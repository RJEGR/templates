...
<!-- COMENTARIOS del Flujo del analisis -->
<!-- Pre-procesamiento y alineamiento de secuencias. Figura 1 a 5 -->
<!-- - Identificación, abundancia y clasificación taxonómica de amplicones. Figura 6 a 10 -->
<!-- Subset de datos: -->
<!-- phyloseq = Todos los datos -->
<!-- physeq =  Subset de datos (por crucero) -->
<!-- clean.obj = Retener amplicones presentes en al menos dos muestras -->
<!-- physeq2 = transformacion de datos del objeto physeq y retener amplicones con variación mayor  a 1e-5 -->
<!-- otra opción (no implementada) es la abundancia Estandarizada a la media de las lecturas secuenciadas. -->

<!-- - Estimación de los índices de diversidad y presentación gráfica de resultados. Figura 11 a 13 y tabla 1. -->
<!-- - Cribado de Peces (Actinoperigy). Figura 14. y Figura 15.  -->
...
```{r quality}
set <- 'G03'
path <- c("~/metagenomics/COI/run012")
qc.dir <- paste0(path, "/quality")
...
ctb.file <- list.files(path, pattern = "run012_relax_ASVs_count.table", full.names = TRUE)
seqtab.nochim <- read.table(ctb.file)
# load(RData.file) # instead of load RData, only read the count_table
...
# Curate taxonomy
tax.dir <- paste0(path, "/ASSIGNATION/asv_analisis")
tax.file <- list.files(tax.dir, pattern = "run012_relax_ASVs_midori_unique_DB_0.wang.wm.taxonomy", full.names = TRUE)
taxa1 <- read.csv(tax.file, header=FALSE, sep="\t", stringsAsFactors=FALSE)

Ranks <- c("Reino", "Filo", "Clase", "Orden", "Familia", "Genero", "Especie") # midori levels

tax <- strsplit(taxa1[,2], ";")
tax <- sapply(tax, "[", c(1:7)) # midori levels
tax <- as.data.frame(t(tax))

taxa <- as.data.frame(apply(tax, 2, 
    function(x) gsub("\\(.*$", "",  x, perl=TRUE)),
     stringsAsFactors = F)

# taxa <- taxa[,-c(1)] # For remove midori `root` Rank
colnames(taxa) <- Ranks 
taxa <- apply(taxa, 2, as.character)
rownames(taxa) <- taxa1[,1]
head(taxa)
...
phyloseq <- phyloseq(otu_table(t(seqtab.nochim), taxa_are_rows=FALSE), 
               sample_data(samples), 
               tax_table(taxa))
...
keepTaxa = apply(X = as(otu_table(physeq), "matrix") > 0, # Removing any abundance of zero
                 MARGIN = 2, FUN = sum) > 2 # Remove ASVs not k greater than k (2L) 
...
```{r GOMECC 03, eval = TRUE, echo = FALSE }
samples.G03 <- sample_data(physeq)
samples.G03$Tipo <-  sub('.*(?=.$)', '', samples.G03$Estación, perl=T)
samples.G03[samples.G03$Tipo == 'H']
samples.G03[samples.G03$Tipo == 'H','Tipo'] <- 'Huevos'
samples.G03[samples.G03$Tipo == 'L','Tipo'] <- 'Larvas'


# etiquetar familias predominantes para fines de pesca.
# colorear curvas de rarefacccion por huevos o larva.
# Separar alfa diversidad por Huevos y Larvas.

# physeq3 <- merge_phyloseq(physeq, samples.G03)


physeq <- phyloseq(otu_table(physeq, taxa_are_rows=FALSE), 
               sample_data(samples.G03), 
               tax_table(taxa))
...
* Figura 14. Taxones con varianza mayor a 1e-5 asignados a la Clase de _Actinoperigy_. 
* Figura 15. Las secuencias de _Actinoperigy_ fueron alineadas y la filogenia construída por el método Neighbor Joining ajustado con el modelo GTR+G+I. La paleta de colores indica la asociación de los taxones a nivel Familia y la etiqueta de los géneros
...
track.dir <- paste0(path, "/run12_20181128_COI/run012_20181204_COI_relax")
...
Sample <- sapply(strsplit(id, "-"), '[', 2)

samples.G03 = data.frame(Sample = Sample, 
              Tipo = sub('.*(?=.$)', '', as.character(Sample), perl=T),
              stringsAsFactors = FALSE)

samples.G03[samples.G03$Tipo == 'H','Tipo'] <- 'Huevos'
samples.G03[samples.G03$Tipo == 'L','Tipo'] <- 'Larvas'

track_out$Sample <- samples.G03$Sample
track_out$Tipo <- samples.G03$Tipo
head(track_out)

library(dplyr)


track_m <- reshape2::melt(track_out, id.vars=c("Sample", "Tipo"),
...
track_plot <- ggplot(track_m, aes(x=Sample, y=Reads, fill=Process)) + 
    geom_col(position = position_identity(), width = 0.7, aes(fill = Process), alpha = 0.7) +
...
mergedf.dir <- paste0(path, "/run12_20181128_COI/run012_20181204_COI_relax")
...
# also for GOMECC, 
id = as.character(merge_df$Sample)
...
samples.G03[samples.G03$Tipo == 'H','Tipo'] <- 'Huevos'
samples.G03[samples.G03$Tipo == 'L','Tipo'] <- 'Larvas'
merge_df$Tipo <- samples.G03$Tipo
merge_df$Sample <- samples.G03$Sample


data <- data.frame(x = merge_df[,'nmatch'], y = merge_df[,'seqlen'], z = merge_df[,'Tipo'])
...
# save <- psmelt(physeq)
colnames(save)[1] <- "ASV"
write.csv(save, file = paste0(set, "_taxones_identificados.csv"), 
          row.names = FALSE, quote = FALSE)
rm(save)
```


```{r pyseq2-transform.sample.counts}
...
clean.obj.st = subset_taxa(clean.obj, Filo!="Arthropoda")
...
obj$data$tax_occ <- calc_n_samples(obj, "tax_abund", groups = sample_dat$Tipo)

set.seed(100) # This makes the plot appear the same each time it is run 

# node_color <- sample(sample_dat$Tipo)[1]
...
#               subtaxa = TRUE) %>%
  ...
  #         node_size = n_obs,
  #         node_color = Larva,
  #         node_size_axis_label = "Larva count",
...
```{r Figura 7. Heatmap.Class, fig.align = "center", fig.cap = "Figura 7. Composición de las comunidades a nivel Clase a lo largo de las estaciones. Se presentan los taxones con varianza mayor a  1e-5.", out.width='\\textwidth', fig.width=8}

Class <-tax_glom(physeq2, taxrank="Clase")
taxaOrder = rownames(sort(unique(tax_table(Class)[,'Clase']), decreasing=TRUE))
...
```{r Figura 10. Boxplot Genus, fig.align = "center", fig.cap = "Géneros más abundantes a lo largo de todas las estaciones. Se muestran aquellos taxones que representan al menos 10% de la abundancia total por muestra.", fig.width = 8, fig.height = 7}
...
glom <- clean.obj # you can also use clean.obj
...
dat[(Abundance <= 10), Especie := "Otros"]

# dat[(median <= 0), Especie := "Otros"]
# group dataframe by Taxa, calculate median rel. abundance
# dat[, median := median(Abundance, na.rm = TRUE), by = "Familia"]
# plot(density(dat$median))

# boxplot
ggplot(dat[Abundance > 0 & Especie != 'NA'],
      aes(x=Especie,
          y=Abundance)) +
 geom_boxplot() +
 coord_flip() +
 #scale_y_log10() + 
 ylim(0, round(max(dat$Abundance))) + ylab("Abundancia Relativa (%)") + 
 theme_minimal() + facet_wrap(~ Tipo, nrow = 2 )
```

```{r Figura N. Bubbleplot, bubbleplot Species, fig.align = "center", fig.cap = "Especies más abundantes a lo largo de todas las estaciones. Se muestran aquellos taxones que representan al menos 10% de la abundancia total por muestra (Se colorean por orden cada una de las especies).", fig.width = 8, fig.height = 7}

library(RColorBrewer)

# colourCount = length(unique(tax_table(physeq)[,'Orden'])) # length(unique(sample_data(physeq)$Tipo))))
# getPalette = colorRampPalette(brewer.pal(12, "Paired")) 


ggplot(dat[Abundance > 0 & Especie != 'Otros' & Especie != 'NA'], 
       aes(x=Estación, 
           y=Especie, 
           color=Orden)) +
...
```{r metacoder.by.group, eval = FALSE, echo = FALSE}

# Error in apply(layout_matrix, MARGIN = 2, function(x) all(is.na(x))) : 
# Comparing two treatments/groups


# La siguientes lineas son para comparar las muestras de difentes grupos, ej. larva y huevo
...
obj$data$diff_table <- compare_groups(obj, dataset = "tax_abund",
                                      ...
                                      groups = sample_dat$Tipo) # What category each sample is assigned to
...
heat_tree_matrix(obj,
                 dataset = "diff_table",
                 # n_obs is a function that calculates, in this case, the number of OTUs per taxon
                 node_size = n_obs, 
                 node_label = taxon_names,
...
```{r funcion rarefaccion, echo = FALSE}
...
rrplot +
  ...
  scale_color_manual(values=getPalette(length(unique(sample_data(physeq)$Tipo))))
```
...
alfa <- physeq
n <-length(unique(sample_data(alfa)$Tipo))
alfa <- transform_sample_counts(alfa, function(x) 1E6 * x/sum(x))

prichnes <- plot_richness(alfa, x="Estación", color="Tipo", measures=c("Shannon", "InvSimpson"))
...
p$layers <- p$layers[-1]
p + geom_text(aes(label = Estación, label.size = 10)) + 
    theme(legend.position = "top")+
    scale_color_manual(values=getPalette(length(unique(sample_data(rr)$Tipo))))
...
```{r Abundance Difference, eval = FALSE, echo = FALSE}

# # #
# Test abundance difference 
# # #

# Remove taxa not seen more than 3 times in at least 10% of the samples. This protects against an ASVs with small mean & trivially large C.V.
GP = phyloseq::filter_taxa(physeq, function(x) sum(x > 3) > (0.2*length(x)), TRUE)
# Standardize abundances to the median sequencing depth
total = median(sample_sums(GP))
standf = function(x, t=total) round(t * (x / sum(x)))
gps = transform_sample_counts(GP, standf)
# Filter the taxa using a cutoff of 3.0 for the Coefficient of Variation
gpsf = phyloseq::filter_taxa(gps, function(x) sd(x)/mean(x) > 3.0, TRUE)
gpsf
...
Para visualizar este grupo de forma independiente (taxones en la Clase _Actinopteri_) se generó un gráfico de abundancia relativa a nivel familiar (Figura 14). En el dendograma de la Figura 15 se observa que las secuencias de los amplicones asignados a las diferentes familias de la Clase _Actinopteri_ no forman grupos definidos (ie. no son monofiléticos).

Para la construcción del árbol, se alinearon los amplicones para la construyó el arbol filogenetico utilizando el método relajado Neihbor-Joining (Evans, J. et al 2006) ajustado con un modelo GTR+G+I (Generalized time-reversible with Gamma rate variation).

```{r Figura 14. Peces, fig.align = "center", fig.cap = "Figura 14. Taxones (nivel Especie) con varianza mayor a 1e-5 asignados a la Clase de Actinoperigy.", fig.width = 7, fig.height = 5}
library(phyloseq)
actii = subset_taxa(physeq2, Clase == "Actinopterygii") #  "Actinopteri"
actii = prune_taxa(taxa_sums(actii) > 0, actii)
actii.glm = tax_glom(actii, taxrank="Especie")

# actii.Ist = subset_taxa(physeq, Familia=="Istiophoridae")
# actii.Scom = subset_taxa(physeq, Familia=="Scombridae")

plotbar = actii.glm

G03.samples <- c(sample_data(actii.glm)$`Estación`[1:22]) # 23:45
plotbar <- subset_samples(actii.Fgl, Tipo == 'Larva')

colourCount = length(unique(tax_table(plotbar)[,'Familia']))
getPalette = colorRampPalette(brewer.pal(9, "Spectral"))

theme_set( theme_classic())

bar <- NULL
bar <- plot_bar(plotbar, "Especie", fill="Orden")  +
       geom_bar(stat = "identity") +
  scale_fill_manual(values = getPalette(colourCount)) +
   ylab("Abundancia Relativa (%)") +  coord_flip() +
  guides(fill=guide_legend(ncol=2)) + facet_wrap(~Estación)
...
```{r redoing ntaxa plot}
actii.df <- as_tibble(phyloseq::psmelt(actii))
# actii.df[complete.cases(actii.df[,Ranks]), ]
actii.df <- actii.df[, c('Estación', 'Tipo', 'Abundance', Ranks)]


lineage <- reshape2::melt(actii.df, id.vars=c('Estación', 'Tipo', 'Abundance'),
                          variable.name = "Rank", 
                          value.name = "lineage")



lineage$Rank <- factor(lineage$Rank, 
                          levels = Ranks,
                          ordered=T)
# by rank
# lineage.stats <- aggregate(lineage[,'Rank'], by=list(lineage[,'lineage']), FUN = table)

# lineage[lineage[,'Rank'] == 'Especie', 'lineage']
# lineage[lineage[,'Rank'] == 'Especie', 'Estación']
# by species
lineage.stats <- aggregate(lineage[lineage[,'Rank'] == 'Especie', 'Estación'], 
                          by=list(lineage[lineage[,'Rank'] == 'Especie', 'lineage']), FUN = table)


lineage.stats <- data.frame(lineage = lineage.stats[,1], lineage.stats[,2])


lineage.datavis <- reshape2::melt(lineage.stats,
                          variable.name = "Estación", # or Rank if by rank
                          value.name = "ntaxa")

plot(density(lineage.datavis$ntaxa))

# second aggregregation of species per station

lineage.stats2 <- aggregate(lineage.datavis[,'ntaxa'], 
                          by=list(lineage.datavis[,'Estación']), FUN = sum)

lineage.stats2 <- data.frame(lineage = lineage.stats2[,1], lineage.stats2[,2])
# :(( no funciono ! revisar
ggplot() + geom_bar(aes(y = ntaxa, x = Rank, fill = lineage), data = lineage.datavis,
                           stat="identity")

```


```{r Figura x. SuperHeat, eval = FALSE, echo = FALSE}
library(superheat)

#Class <- NULL
#Class <- tax_glom(physeq, taxrank="Clase")
#Class <- transform_sample_counts(physeq, function(x) (x / sum (x) * 100) )


heat.tbl <- as.matrix(otu_table(Class))
rownames(heat.tbl) <- sample_data(Class)$`Estación`
colnames(heat.tbl) <- tax_table(Class)[,'Clase']
yr <- sample_sums(physeq) # # log10(sample_sums(physeq)),
heat.samples <- sample_data(Class)$Tipo
heat.samples[heat.samples == 'Larvas'] <- '#E41A1C'
heat.samples[heat.samples == 'Huevos'] <- '#999999'
...
superheat(heat.tbl, 
          ...
          yr = yr,
          yr.obs.col =  heat.samples,
          yr.axis.name = "Numero de Secuencias",
          yr.plot.type = "bar",
          # set bar colors
          yr.bar.col = "black",
          # Titles
          column.title = 'Taxones (Clase)',
          column.title.size = 4,
          row.title = 'Muestras',
          row.title.size = 4,
          # remove the grid
...
```{r Gomec03, eval = FALSE, echo = FALSE}
G03.samples2 <- as.character(sample_data(actii.Fgl)$`Estación`[23:nsamples(actii.Fgl)])
plotbar <- NULL
plotbar <- subset_samples(actii.Fgl, Estación = G03.samples) 
...
bar <- NULL
bar <- plot_bar(plotbar, "Familia", fill="Familia")  +
       geom_bar(stat = "identity") +
  scale_fill_manual(values = getPalette(colourCount)) +
   ylab("Abundancia Relativa (%)") +  coord_flip() +
  guides(fill=guide_legend(ncol=2)) + facet_wrap(~Estación)
...
```{r Tree construction, echo = FALSE, warning=FALSE, echo = FALSE, message = FALSE}
...
fasta.dir <- c("~/metagenomics/COI/run012/")
fasta.file <- list.files(fasta.dir, pattern = "run012_relax_ASVs.fasta", full.names = TRUE)
...
actii.df <- as_tibble(phyloseq::psmelt(actii))
colnames(actii.df)[1] <- "label"

df <- actii.df %>% group_by(label) %>% 
  filter (!duplicated(label)) %>% 
  select(label, Sample, Clase, Orden, Familia, Genero)

seqs.subset <- seqs[names(seqs) %in% rownames(tax_table(actii))]
...
ggtree(tre, branch.length='none', layout='slanted', aes(color= Familia, fill=Familia), na.rm = FALSE) +
  theme(legend.position="right") +
  scale_color_manual(values = c(getPalette(colourCount)), 
                     na.value = "grey", 
                     guide = guide_legend(ncol=1)) +
  xlim(0, 30) +
  geom_tiplab(size=1, aes(label=Familia), offset=0.5) +
